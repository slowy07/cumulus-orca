...
import request_status_for_granule_dummy
...
class TestRequestStatusForGranuleUnit(
    ...
    def test_task_output_json_schema(self):
        ...
        """
        result = request_status_for_granule_dummy.handler(
            {
                request_status_for_granule_dummy.INPUT_GRANULE_ID_KEY: uuid.uuid4().__str__(),
                request_status_for_granule_dummy.INPUT_JOB_ID_KEY: uuid.uuid4().__str__()
            }, Mock()
        )
